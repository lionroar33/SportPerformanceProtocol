...
export class BlockchainService {
  ...
  private isValidSolanaAddress(address: string): boolean {
    // Basic validation - Solana addresses are base58, typically 32-44 chars
  ...
  async recordVerifiableDataPoint(matchId: string, data: any): Promise<string> {
    // In production, this would write to Solana blockchain
    const txHash = `vdp_${Date.now().toString(16)}${Math.random().toString(16).slice(2)}`;

    this.logger.log(`Verifiable data point recorded for match ${matchId}: ${txHash}`);
    return txHash;
  }
